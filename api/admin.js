...
module.exports.load = async function(app, db) {
    ...
    app.get("/addresources", async (req, res) => {
        ...
        if (req.query.ram || req.query.disk || req.query.cpu || req.query.servers) {
            ...
            adminjs.suspend(req.query.id);
            console.log(chalk.white("[🛡️ADMIN] ") + chalk.white((`add resources`, `${req.session.userinfo.username}#${req.session.userinfo.discriminator} added the resources of the user with the ID \`${id}\` value:\`\`\`\nSLOTS: ${serversstring}\nCPU: ${cpustring}%\nRAM: ${ramstring} MB\nDISK: ${diskstring} MB\`\`\``)))
            return res.redirect(successredirect + "?err=none");
        } else {
            res.redirect(`${failredirect}?err=MISSINGVARIABLES`);
        }

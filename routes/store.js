...
module.exports.load = async function(app, db) {
  ...
  app.get("/buyram", async (req, res) => {
    ...
    if (newsettings) {
      ...
      console.log(`${req.session.userinfo.username}#${req.session.userinfo.discriminator} bought ${per}\MB ram from the store for \`${cost}\` Credits.`)
  ...
  app.get("/buydisk", async (req, res) => {
    ...
    if (newsettings) {
      ...
      console.log(`${req.session.userinfo.username}#${req.session.userinfo.discriminator} bought ${per}MB disk from the store for \`${cost}\` Credits.`)
  ...
  app.get("/buycpu", async (req, res) => {
    ...
    if (newsettings) {
      ...
      console.log(`${req.session.userinfo.username}#${req.session.userinfo.discriminator} bought ${per}% CPU from the store for \`${cost}\` Credits.`)
  ...
  app.get("/buyservers", async (req, res) => {
    ...
    if (newsettings) {
      ...
      console.log(`${req.session.userinfo.username}#${req.session.userinfo.discriminator} bought ${per} Slots from the store for \`${cost}\` Credits.`)
  ...
  app.get("/buybackups", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if (backupscap + amount > settings.storelimits.backups) return res.redirect(failedcallback + "?err=MAXSERVERSEXCEETED");
      ...
      if(newbackups > settings.storelimits.backups) return res.send("Reached max server limit!");
      if (newusercoins == 0) {
        await db.delete("coins-" + req.session.userinfo.id);
      ...
      if (extra.ram == 0 && extra.disk == 0 && extra.cpu == 0 && extra.servers == 0 && extra.backups == 0 && extra.databases == 0) {
        await db.delete("extra-" + req.session.userinfo.id);
      } else {
        await db.set("extra-" + req.session.userinfo.id, extra);
      }
      ...
      log(`Resources Purchased`, `${req.session.userinfo.username}#${req.session.userinfo.discriminator} bought ${per} Slots from the store for \`${cost}\` Credits.`)
      console.log(`${req.session.userinfo.username}#${req.session.userinfo.discriminator} bought ${per} Backups from the store for \`${cost}\` Credits.`)
      res.redirect((theme.settings.redirect.purchaseservers ? theme.settings.redirect.purchaseservers : "/") + "?err=none");
    }
  });
  ...
  app.get("/buydatabases", async (req, res) => {
    ...
    if (newsettings) {
      ...
      if (databasescap + amount > settings.storelimits.databases) return res.redirect(failedcallback + "?err=MAXSERVERSEXCEETED");
      ...
      if(newdatabases > settings.storelimits.databases) return res.send("Reached max server limit!");
      if (newusercoins == 0) {
        await db.delete("coins-" + req.session.userinfo.id);
      ...
      log(`Resources Purchased`, `${req.session.userinfo.username}#${req.session.userinfo.discriminator} bought ${per} Slots from the store for \`${cost}\` Credits.`)
      console.log(`${req.session.userinfo.username}#${req.session.userinfo.discriminator} bought ${per} Databases from the store for \`${cost}\` Credits.`)
      res.redirect((theme.settings.redirect.purchaseservers ? theme.settings.redirect.purchaseservers : "/") + "?err=none");
    }
  });

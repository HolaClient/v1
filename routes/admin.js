...
module.exports.load = async function(app, db) {
    ...
    app.get("/setresources", async (req, res) => {
        ...
        if (req.query.ram || req.query.disk || req.query.cpu || req.query.servers) {
            ...
            console.log(chalk.white("[🛡️ADMIN] ") + chalk.white((`set resources`, `${req.session.userinfo.username}#${req.session.userinfo.discriminator} set the resources of the user with the ID \`${id}\` to:\`\`\`\nSLOTS: ${serversstring}\nCPU: ${cpustring}%\nRAM: ${ramstring} MB\nDISK: ${diskstring} MB\`\`\``)))
    ...
    app.get("/remove_account", async (req, res) => {
        ...
        console.log(`${req.session.userinfo.username}#${req.session.userinfo.discriminator} removed the account with the ID \`${discordid}\`.`)
        res.redirect(theme.settings.redirect.removeaccountsuccess + "?err=REMOVEACCOUNT");
    });
    ...
    app.get("/getip", async (req, res) => {
        ...
        let failredirect = theme.settings.redirect.failedgetip || "/admin";
